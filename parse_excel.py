...
def range_slice(range_str):
    ...
    """
    (start_row, start_col), (stop_row, stop_col) = [xl_cell_to_rowcol(c) for c in range_str.split(':')]
...
def parse_length(length, dict):
    """
    根据传入的长度值返回相应的长度。

    Args:
        length (int|str): 传入的长度值，可以是整数类型或字符串类型。
        dict (dict): 一个字典，用于将字符串类型的长度值映射到对应的整数长度值。

    Returns:
        int: 返回解析后的长度值，如果传入的length是整数类型，则直接返回该整数；
             如果传入的length是字符串类型，则从dict中查找对应的整数长度值并返回。

    Raises:
        KeyError: 如果传入的length是字符串类型，并且在dict中找不到对应的键，则会引发KeyError异常。
    """
